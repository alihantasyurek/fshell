...
/*   Updated: 2024/03/15 19:07:26 by atasyure         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_builtin_command(t_parse *parse, t_mini *m_mini)
{
	...
	new_parse = parse;
	if ((ft_strcmp(new_parse->cmd, "echo") == 0) || (ft_strcmp(new_parse->cmd, "ECHO") == 0))
		exec_echo(new_parse, m_mini);
	if (ft_strcmp(new_parse->cmd, "cd" ) == 0|| (ft_strcmp(new_parse->cmd, "CD") == 0)) //changed
...
int	is_builtin(t_parse *data)
{
	...
	if ((ft_strcmp(data->cmd, "env") == 0) || (ft_strcmp(data->cmd,
				"ENV") == 0))
		return (1);
	if ((ft_strcmp(data->cmd, "cd") == 0) || (ft_strcmp(data->cmd, "CD") == 0)) //changed
		return (1);
